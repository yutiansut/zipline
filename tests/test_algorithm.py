...
class TestPositions(WithLogger,
    ...
    def test_empty_portfolio(self):
        algo = EmptyPositionsAlgorithm(self.asset_finder.equities_sids,
                                       sim_params=self.sim_params,
                                       env=self.env)
        perf = algo.run(self.data_portal).daily_performance
        ...
        assert_equal(expected_count, perf.longs_count.values)
...
class TestEquityAutoClose(WithTradingEnvironment, WithTmpDir, ZiplineTestCase):
    ...
    def test_daily_delisted_equities(self,
        ...
        start_date = resources.sim_params.start_session
        ...
        if order_size > 0:
            self.assertEqual(
                ...
                daily_perf.longs_count.tolist(),
            )
            self.assertEqual(
                [0] * len(self.test_days),
                daily_perf.shorts_count.tolist(),
            )
        else:
            self.assertEqual(
                expected_num_positions,
    ...
    def test_minutely_delisted_equities(self):
        ...
        self.assertEqual(
            output.daily_performance.longs_count.tolist(),
            [3, 3, 3, 2, 2, 1, 1],
        )
